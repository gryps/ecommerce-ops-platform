...
DB = BASE / 'wms.db'
DATABASE_URL = os.environ.get('WMS_DATABASE_URL', '').strip()
...
class PgConnection:
    def __init__(self, url):
        if psycopg is None:
            raise RuntimeError('psycopg is required when WMS_DATABASE_URL uses PostgreSQL')
    ...
    def __exit__(self, exc_type, exc, tb):
        return self.con.__exit__(exc_type, exc, tb)

    def execute(self, sql, args=()):
        return self.con.execute(sql.replace('?', '%s'), args)

    def executemany(self, sql, args):
        statement = sql.replace('?', '%s')
        for row in args:
            self.con.execute(statement, row)
...
def init_db():
    ...
    with db() as con:
        ...
        log_cols = table_columns(con, 'inventory_logs')
        ...
        try:
            with db() as con:
                if data.get('id'):
                    ...
                    con.execute('insert into products(sku,code,name,location,stock,min_stock,unit) values(?,?,?,?,?,?,?)', vals)
                    pid = con.execute('select id from products where sku=?', (sku,)).fetchone()['id']
        except Exception as exc:
            if not db_integrity_error(exc):
                raise
