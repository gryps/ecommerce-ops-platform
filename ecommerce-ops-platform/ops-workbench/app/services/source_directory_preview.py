...
from app.media import IMAGE_EXTENSIONS, VIDEO_EXTENSIONS, resolve_source_directory
...
def select_native_image_files(initial_path: str = "") -> tuple[str, list[Path]]:
    powershell = shutil.which("powershell.exe")
    if not powershell:
        windows_powershell = Path(
            "/mnt/c/WINDOWS/System32/WindowsPowerShell/v1.0/powershell.exe"
        ...
        if windows_powershell.is_file():
            powershell = str(windows_powershell)
    if not powershell:
        raise RuntimeError("当前系统无法调用 Windows 文件选择窗口")
    initial_windows = ""
    if initial_path:
        try:
            initial_windows = _windows_path(
...
public static class ImageFileDialogForeground {
    private delegate bool EnumWindowsProc(IntPtr hWnd, IntPtr lParam);
    ...
    public static void ForceDialogToFront(int processId) {
        EnumWindows(delegate(IntPtr hWnd, IntPtr lParam) {
            uint ownerProcessId;
            GetWindowThreadProcessId(hWnd, out ownerProcessId);
            if (ownerProcessId != (uint)processId || !IsWindowVisible(hWnd)) {
...
'@
$dialog = New-Object System.Windows.Forms.OpenFileDialog
$dialog.Title = '选择一个或多个商品实拍图'
$dialog.Multiselect = $true
$dialog.CheckFileExists = $true
$dialog.RestoreDirectory = $true
$dialog.Filter = '图片文件|*.jpg;*.jpeg;*.png;*.webp;*.bmp;*.tif;*.tiff|所有文件|*.*'
$initial = '__INITIAL_PATH__'
if ($initial -and (Test-Path -LiteralPath $initial)) {
    $dialog.InitialDirectory = $initial
}
...
$foregroundTimer.Interval = 120
$foregroundTimer.Add_Tick({
    [ImageFileDialogForeground]::ForceDialogToFront($PID)
})
$foregroundTimer.Start()
$result = $dialog.ShowDialog($owner)
$foregroundTimer.Stop()
...
""".replace("__INITIAL_PATH__", escaped_initial)
    try:
        ...
        raise RuntimeError("文件选择窗口等待超时") from exc
    except subprocess.CalledProcessError as exc:
        message = (exc.stderr or "").strip()
        raise RuntimeError(f"无法打开系统文件选择窗口：{message or 'PowerShell 执行失败'}") from exc
    selected = [line.strip() for line in result.stdout.splitlines() if line.strip()]
    if not selected:
        return "", []
    try:
        images = [resolve_source_image(_linux_path(value)) for value in selected]
        common_parent = Path(
            os.path.commonpath([str(image.parent) for image in images])
        )
        folder = resolve_source_directory(str(common_parent))
        images = sorted(images, key=lambda path: path.relative_to(folder).as_posix().lower())
        return str(folder), images
    except (ValueError, OSError, subprocess.SubprocessError) as exc:
        raise RuntimeError(f"所选实拍图不可用于图片生产：{exc}") from exc


def resolve_source_image(path: str) -> Path:
    try:
        image = Path(path).expanduser().resolve(strict=True)
    except OSError as exc:
        raise ValueError("实拍图不存在或不可访问") from exc
    if not image.is_file() or image.suffix.lower() not in IMAGE_EXTENSIONS:
        raise ValueError("文件不是支持的商品实拍图")
    resolve_source_directory(str(image.parent))
    return image
